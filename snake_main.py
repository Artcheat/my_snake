...
def clicked(event):
    ...
    c.itemconfigure(exit_game, state='hidden')
    start_game()
...
restart_text = c.create_text(WIDTH / 2, HEIGHT - HEIGHT / 3,
                             ...
                             state='hidden')
exit_game = c.create_text(WIDTH / 2, HEIGHT / 4, text='НАЖМИТЕ ESC ДЛЯ ВЫХОДА', fill='black', font='Impact 10',
                          state='hidden')

c.tag_bind(restart_text, "<Button-1>", clicked)
c.tag_bind(exit_game, "<Escape>", clicked)
start_game()
root.mainloop()
